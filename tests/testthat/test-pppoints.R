test_that("PP points works with two numeric.", {
  ## Default `a`
  res <- as.data.frame(pppoints(1:5, 1:18))
  check <- data.frame(
  ...
  res <- as.data.frame(pppoints(x, 1:18, data = dat))
  expect_equal(res, check)
  dat <- data.frame(y = 1:18)
  res <- as.data.frame(pppoints(1:5, y, data = dat))
  expect_equal(res, check)
  ## Try a different `a`
  res <- as.data.frame(pppoints(1:3, 1:4, a = 1.5))
  check <- data.frame(
  ...
  res <- as.data.frame(pppoints(1:3, 1:4, ngrid = 5, a = 1.5))
  check <- data.frame(
...
test_that("Duplicate data still works.", {
  res <- as.data.frame(pppoints(c(2, 2, 3.5), 1:4, a = 1.5))
  check <- data.frame(
...
test_that("PP points errors out appropriately.", {
  expect_error(pppoints(data.frame(x = 1), 4))
  expect_error(pppoints(distionary::dst_norm(0, 1), data.frame(x = 1)))
  expect_error(pppoints(distionary::dst_norm(0, 1),
                        distionary::dst_exp(1)))
})

test_that("PP points returns a zero-row data frame when appropriate.", {
  expect_equal(nrow(pppoints(numeric(0), 1:10)), 0)
  expect_equal(nrow(pppoints(numeric(0), 1:10, ngrid = 10)), 0)
  expect_equal(nrow(pppoints(1:10, numeric(0))), 0)
  expect_equal(nrow(pppoints(1:10, numeric(0), ngrid = 10)), 0)
})

test_that("PP points works with two distributions.", {
  res <- as.data.frame(pppoints(distionary::dst_norm(0, 1),
...
test_that("PP points works with one numeric, one distribution.", {
  res <- as.data.frame(pppoints(1:5, distionary::dst_exp(1)))
  check <- data.frame(prob_x = uscore(1:5),
                      prob_y = stats::pexp(1:5))
  expect_equal(res, check)
  res <- as.data.frame(pppoints(distionary::dst_exp(1), 1:5))
...
test_that("PP knows what to do when finite distributions are supplied.", {
  ...
  y <- c(1.5, 2, 3)
  res <- as.data.frame(pppoints(x, y))
  check <- data.frame(
